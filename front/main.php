<div class="news_btns d-f">
    <div class="news_btn activeBtn" onclick="show_news(1,this)">健康新知</div>
    <div class="news_btn" onclick="show_news(2,this)">菸害防制</div>
    <div class="news_btn" onclick="show_news(3,this)">癌症防治</div>
    <div class="news_btn" onclick="show_news(4,this)">慢性病防治</div>
</div>
<div class="news_list">
    <div class="news_item activeNews" data-id="1">
        <h1>健康新知</h1>
        <pre>
        資料來源： 行政院衛生署國民健康局
        發佈日期： 2012 / 10 / 07
...
<script>
function show_news(id,e) {
    $('.news_item').removeClass('activeNews');
    $('.news_btn').removeClass('activeBtn');

    $('.news_item').each((key, val) => {
        if ($(val).data('id') == id) {
            $(val).addClass('activeNews');
